...
def generate_shifted_steps(num_substeps: int, max_step: int, shift: int) -> np.ndarray:
    # return np.linspace(max_step - 1, shift, num_substeps, endpoint=False).astype(int)[::-1]

    substeps = np.linspace(1, 0, num_substeps, endpoint=False)
    # substeps = np.linspace((max_step - 1) / max_step, 0, num_substeps, endpoint=False)

    substeps = (shift * substeps) / (1 + (shift - 1) * substeps)
    substeps = substeps * max_step - 1  # * Max from 1000 -> 999
    substeps = substeps.astype(int)[::-1]

    # shift=7, substeps
    # array([124, 224, 307, 377, 436, 487, 531, 570, 604, 635, 662, 687, 709,
    #    730, 748, 766, 781, 796, 809, 822, 834, 845, 855, 864, 874, 882,
    #    890, 898, 905, 912, 918, 924, 930, 936, 941, 946, 951, 955, 960,
    #    964, 968, 972, 976, 979, 983, 986, 989, 993, 996, 999])
...
class ZeroSNRDDPMDiscretization(Discretization):
    def __init__(
        ...
        self.sd3_time_shift = sd3_time_shift

    def get_sigmas(self, n, device="cpu", return_idx=False):
        if n < self.num_timesteps:
            
            if self.sd3_time_shift is not None:
                timesteps = generate_shifted_steps(n, self.num_timesteps, self.sd3_time_shift)   # * shift here.

            else:
                timesteps = generate_roughly_equally_spaced_steps(n, self.num_timesteps)   # * equal space here. need to change.
