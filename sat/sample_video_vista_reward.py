...
def save_text(text: str, save_path: str, output_name="text.txt"):
    txt_save_path = os.path.join(save_path, output_name)
            ...
            for index in range(args.batch_size):
                ...
                for ind_round in range(n_prediction_round):
                    ...
                    for _ in range(ENSEMBLE_SIZE):
                        ...
                        sample_reward = samples_z.flatten(0, 1)  # * b*t, c, h, w
                        sample_ensemble.append(sample_reward)

                    u = torch.mean(torch.stack(sample_ensemble), 0)
                    diff = torch.zeros_like(sample_reward)
                    for each_sample in sample_ensemble:
                        diff.add_((each_sample - u) ** 2)
                    variance = diff / (ENSEMBLE_SIZE - 1)
                    # TODO: check the shape of variance
                    # torch.Size([25, 4, 72, 128])

                    vista_reward = torch.exp(-variance.mean()).cpu()
                    
                    print("\n!!!!! Vista Reward:", vista_reward)
                    print()
                    
                    # import pdb; pdb.set_trace()
                    
                    # samples_z = samples_z.permute(0, 2, 1, 3, 4).contiguous()
                    # # samples_z: torch.Size([1, 16, 13, 64, 112]) b, c, t, h, w
                ...
                if mpu.get_model_parallel_rank() == 0:
                    ...
                    save_text(f"{vista_reward}", save_path, output_name="reward.txt")
