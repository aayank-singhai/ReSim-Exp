...
if __name__ == "__main__":
    ...
    if os.path.isfile(args.video_path):
        ...
        save_path = args.video_path.replace('gt', 'rec')
        save_video(output, save_path, fps=FPS)
    else:
        # * folder
        for root, dirs, files in os.walk(args.video_path):
            for file in files:
                if file.endswith(".mp4") and file.startswith('gt'):
                    output = vae_demo(args.model_path, os.path.join(root, file), dtype, device)
                    save_path = os.path.join(root, file).replace('gt', 'rec')
                    save_video(output, save_path, fps=FPS)
