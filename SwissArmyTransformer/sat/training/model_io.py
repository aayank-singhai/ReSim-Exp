...
def get_checkpoint_name(checkpoints_path, iteration, release=False, zero=False, use_ema=False):
    if release:
        d = 'release'
    else:
...
def load_checkpoint(model, args, load_path=None, prefix='', specific_iteration=None):
    ...
    if load_path.endswith('.pt'):
        ...
        iter_str = checkpoint_name.split('/')[-2]
        if iter_str.isdigit():
        ...
        checkpoint_name = get_checkpoint_name(load_path, iteration, release, use_ema=args.use_ema)
        if mpu.get_data_parallel_rank() == 0:
    ...
    # * Remove pretrained pos embedding (the pos embs are not learned, but sin-cos)
    REMOVE_POS = True
    if REMOVE_POS:
        pos_emb_str = 'model.diffusion_model.mixins.pos_embed.pos_embedding'
        if pos_emb_str in sd['module']:
            del sd['module'][pos_emb_str]
