...
class CustomWandbLoggerHook(LoggerHook):
    ...
    def before_run(self, runner):
        ...
        if self.wandb is None:
            self.import_wandb()
        if self.init_kwargs:
            self.init_kwargs['dir'] = runner.work_dir
            if 'name' not in self.init_kwargs:
                if runner.meta is not None and 'exp_name' in runner.meta:
                    self.init_kwargs['name'] = runner.meta["exp_name"].split('.py')[0]
    ...
    def after_run(self, runner):
        if self.log_artifact:
            ...
            for filename in scandir(runner.work_dir, self.out_suffix, False):
